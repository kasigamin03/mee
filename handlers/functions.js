...
function stations(client, prefix, message) {
  try {
    let amount = 0;
    const stationsembed = new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle("VOKEY | RADIO")
    let United_Kingdom = "";
    for (let i = 0; i < radios.EU.United_Kingdom.length; i++) {
      United_Kingdom += `**${i + 1 + 7 * amount}**[${radios.EU.United_Kingdom[i].split(" ")[0].replace("-", " ").substr(0, 16)}](${radios.EU.United_Kingdom[i].split(" ")[1]})\n`;
    }
    stationsembed.addField("RADIO 1", `>>> ${United_Kingdom}`, true);
    amount++;
    let austria = "";
    ...
    stationsembed.addField("RADIO 2", `>>> ${austria}`, true);
        ...
        return setTimeout(() => {
          try {
            ...
            if (player.queue.size === 0) {
              ...
              try {
                embed.setDescription(`I left the Channel: ${client.channels.cache.get(player.voiceChannel).name} because the Queue was empty for`)
              } catch {}
              try {
  ...
  try {
    ...
    function reset(track_info_msg, queue_info_msg) {
      let embed2 = new MessageEmbed()
        ...
        .setTitle("Vokey Music | Music Queue")
      ...
      let embed3 = new MessageEmbed()
        .setColor(ee.color)
        .setFooter(ee.footertext, ee.footericon)
        .setTitle("Vokey Music | Currently no song is playing!")
        .setDescription(`> Join a voice channel and enter a song name or url to play.\n\n[Invite Vokey Music](https://discord.com/oauth2/authorize?client_id=854053203334791238&scope=bot&permissions=4331671048) • [Support Server](https://discord.gg/YFkSGbBUNK) • [Website](https://www.vokey.tk)`)
    ...
    function QueueEmbed(client, player) {
      ...
      const embed = new MessageEmbed().setAuthor(`Vokey Music | Music Queue`);
      const multiple = 15;
      const page = 1;
